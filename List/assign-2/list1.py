...
# Problem Statement
# A trekking company records the elevation (in meters) reached by a hiker
#  at different checkpoints during a mountain climb.
# A checkpoint is considered a peak checkpoint if its elevation is not
#  smaller than its adjacent checkpoints.
...
n = int(input("enter the no of elevations:"))
elev =[]
for i in range(n):
    elev.append(int(input("enter the elevation points:")))
if len(elev)==1:
    print(0)
else:
