...
# A number is called Happy if repeatedly replacing it
# by the sum of squares of its digits eventually
# becomes 1.
...
# ---
# # ---left
elem =[int(x) for x in input("enter input :").split()]
l = len(elem)
largest=0
happ_no_list=[]
...
for i in range(l):
    ...
    while num!=1 and num not in exist:
        exist.add(num)
        res=0
        temp=num
        while temp>0:
        ...
        num = res
    if num ==1:
       happ_no_list.append(elem[i])

print("Happy Numbers =",happ_no_list) 
print("Count = ",len(happ_no_list))

if len(happ_no_list)>0:
   largest = max(happ_no_list) 
