...
# An online shopping website is running a Festive Sale. 
# Customers who purchase products worth ₹2,000 or more are eligible for a 20% discount.
...
prices = map(int,input("ENTer the prices:").split())
eligible = list(filter(lambda x:x>=2000,prices))
discount = list(map(lambda x:x*0.8,eligible))
print(discount)